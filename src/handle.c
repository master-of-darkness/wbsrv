// some parts of code took from h2o library
...
struct st_wbsrv_sendfile_generator_t {
...
struct st_wbsrv_file_handler_t {
    h2o_handler_t super;
...
struct st_wbsrv_specific_file_handler_t {
    h2o_handler_t super;
...
static void close_file(struct st_wbsrv_sendfile_generator_t *self) {
...
static void on_generator_dispose(void *_self) {
    struct st_wbsrv_sendfile_generator_t *self = _self;
    close_file(self);
}
...
static int do_pread(h2o_sendvec_t *src, void *dst, size_t len) {
    ...
    while (bytes_read < len) {
        while ((rret = pread(self->file.ref->fd, dst + bytes_read, len - bytes_read, *file_chunk_at)) == -1 && errno ==
               EINTR);
        if (rret <= 0)
            return 0;
        bytes_read += rret;
...
static void do_proceed(h2o_generator_t *_self, h2o_req_t *req) {
    ...
    struct st_wbsrv_sendfile_generator_t *self = (void *) _self;
    h2o_sendvec_t vec;
...
static void do_multirange_proceed(h2o_generator_t *_self, h2o_req_t *req) {
    ...
    while ((rret = pread(self->file.ref->fd, self->ranged.multirange_buf + used_buf, rlen, self->file.off)) == -1 &&
           errno == EINTR);
    if (rret == -1)
        goto Error;
    self->file.off += rret;
...
#undef TRY_VARIANT
    ...
    if ((flags & H2O_FILE_FLAG_GUNZIP) != 0 && req->version >= 0x101) {
        ...
        if ((fileref = h2o_filecache_open_file(req->conn->ctx->filecache, variant_path, O_RDONLY | O_CLOEXEC)) !=
            NULL) {
            gunzip = 1;
            goto Opened;
        }
...
static void add_headers_unconditional(struct st_wbsrv_sendfile_generator_t *self, h2o_req_t *req) {
...
static void send_decompressed(h2o_ostream_t *_self, h2o_req_t *req, h2o_sendvec_t *inbufs, size_t inbufcnt,
...
static void wbsrv_do_send_file(struct st_wbsrv_sendfile_generator_t *self, h2o_req_t *req, int status, const char *reason,
    ...
    if (self->ranged.range_count == 0)
        ...
        content_range.len = sprintf(content_range.base, "bytes %zd-%zd/%zd", self->ranged.range_infos[0],
                                    self->ranged.range_infos[0] + self->ranged.range_infos[1] - 1,
                                    self->ranged.filesize);
        h2o_add_header(&req->pool, &req->res.headers, H2O_TOKEN_CONTENT_RANGE, NULL, content_range.base,
                       content_range.len);
    }
...
int wbsrv_file_send(h2o_req_t *req, int status, const char *reason, const char *path, h2o_iovec_t mime_type, int flags) {
    ...
    wbsrv_do_send_file(self, req, status, reason, mime_type, NULL, 1);
    return 0;
}
...
static int delegate_dynamic_request(h2o_req_t *req, h2o_iovec_t script_name, h2o_iovec_t path_info,
...
static int try_dynamic_request(wbsrv_file_handler_t *self, h2o_req_t *req, char *rpath, size_t rpath_len) {
    ...
    switch (mime_type->type) {
        ...
        case H2O_MIMEMAP_TYPE_DYNAMIC: {
            h2o_iovec_t script_name = h2o_iovec_init(req->path_normalized.base,
                                                     self->conf_path.len + slash_at - self->real_path.len);
            h2o_iovec_t path_info =
                    h2o_iovec_init(req->path_normalized.base + script_name.len,
                                   req->path_normalized.len - script_name.len);
...
static int serve_with_generator(struct st_wbsrv_sendfile_generator_t *generator, h2o_req_t *req,
                                h2o_iovec_t resolved_path,
    ...
    if ((if_none_match_header_index = h2o_find_header(&req->headers, H2O_TOKEN_IF_NONE_MATCH, -1)) != -1) {
        ...
        if (h2o_filecache_compare_etag_strong(if_none_match->base, if_none_match->len, etag, etag_len))
            goto NotModified;
    } else if ((if_modified_since_header_index = h2o_find_header(&req->headers, H2O_TOKEN_IF_MODIFIED_SINCE, -1)) != -
               1) {
    ...
    if ((range_header_index = h2o_find_header(&req->headers, H2O_TOKEN_RANGE, -1)) != -1) {
        ...
        if (range_count == 1)
            ...
            generator->ranged.mimetype = h2o_strdup(&req->pool, mime_type->data.mimetype.base,
                                                    mime_type->data.mimetype.len);
    ...
    if (rpath[rpath_len - 1] == '/') {
        ...
        for (index_file = self->index_files; index_file->base != NULL; ++index_file) {
            ...
            if (is_dir) {
                ...
                h2o_iovec_t dest = h2o_concat(&req->pool, req->path_normalized, *index_file,
                                              h2o_iovec_init(H2O_STRLIT("/")));
                dest = h2o_uri_escape(&req->pool, dest.base, dest.len, "/");
                if (req->query_at != SIZE_MAX)
                    dest =
                            h2o_concat(&req->pool, dest,
                                       h2o_iovec_init(req->path.base + req->query_at, req->path.len - req->query_at));
